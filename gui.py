...
from training import Board  # Board/game logic from training.py
from model import MCTS, NeuralNetwork  # AI components from model.py

# -------------------- Global Constants --------------------
BOARD_RADIUS = 6  # must match the constant used in training.py
HEX_SIZE = 40
WINDOW_WIDTH = 800
WINDOW_HEIGHT = 800
...
BOARD_CELL_FILL = (150, 75, 0)   # Board fill color
BOARD_BORDER_COLOR = (0, 0, 255)  # Board border color
BG_COLOR = WHITE
TEXT_COLOR = (20, 20, 20)
...
def polygon_corners(center, size):
    """Calculate the pixel positions of hexagon corners."""
    corners = []
    for i in range(6):
...
def draw_text(surface, text, pos, font_size=24, color=TEXT_COLOR):
    """Render text onto the given surface."""
    font = pygame.font.SysFont("arial", font_size)
    text_surface = font.render(text, True, color)
    surface.blit(text_surface, pos)

def draw_board(screen, board, cell_positions):
    """Render the hex board and discs."""
...
def precompute_cell_positions(board, origin):
    """Precalculate pixel positions for each board cell based on axial coordinates."""
    positions = {}
...
def play_game(neural_net, human_player=None):
    ...
    Launch the game window.
      - If human_player is None, all players are controlled by AI.
      - Otherwise, human_player (0, 1, or 2) is controlled by the human.
    """
    board = Board()
    current_player = 0
